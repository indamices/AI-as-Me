...
在使用 DeepSeek 模型时遇到 401 错误：
```
"Provider returned 401", "Incorrect API key provided"
```

错误信息指向 OpenAI 平台，说明 Cursor IDE 可能在验证时使用了错误的端点。

## 问题原因

Cursor IDE 在某些版本中，即使设置了 `baseURL`，在验证 API Key 时仍可能使用默认的 OpenAI 端点，导致验证失败。
...
2. **找到 Models 配置**
   - 在设置搜索框中输入 "Models" 或 "cursor.models"
...
4. **保存并测试**
   ...
   - 如果验证成功，显示 "✓ Connection successful"

5. **重复步骤添加第二个模型**
   - 同样的方式添加 `deepseek-coder`
   - 使用相同的 API Key 和 Base URL
   - Model 设置为 `deepseek-coder`

### 方案 2：修改配置文件（已尝试）

我已经将配置文件中的 `provider` 从 `"openai"` 改为 `"custom"`，但可能仍需要在 UI 中配置才能完全生效。
...
2. **测试 API Key 直接调用**
   ...
   **如果这个测试成功，说明 API Key 是有效的，问题出在 Cursor IDE 的配置上。**
...
如果以上方案都无效，可能是 Cursor IDE 的 bug 或限制：

1. **使用 Cursor 的默认模型**（Claude/GPT）
   - 暂时使用 Cursor 内置的模型

2. **使用其他工具**
   - 使用支持 DeepSeek 的其他 IDE 插件
   - 或者直接使用 DeepSeek 的 Web 界面

3. **报告问题**
   - 在 Cursor IDE 的 GitHub 或论坛报告此问题
   - 提供详细的错误信息和配置
...
2. **检查模型列表**
   ...
   - 查看模型选择下拉菜单
   - 应该能看到 `deepseek-chat` 和 `deepseek-coder`
...
A: 这可能是 Cursor IDE 的 bug。在某些版本中，Cursor IDE 在验证 API Key 时会忽略 `baseURL`，直接使用默认端点。解决方法是使用 UI 界面配置，而不是直接修改 `settings.json`。

### Q: UI 界面中没有 "Models" 选项？

A: 可能是 Cursor IDE 版本太旧，或者界面布局不同。尝试：
- 搜索 "model" 或 "api"
- 查看设置中的 "Features" 部分
- 更新 Cursor IDE 到最新版本
...
A: 这确认了问题出在 Cursor IDE 的配置上，而不是 API Key。尝试：
- 通过 UI 界面重新配置
- 检查 Cursor IDE 版本并更新
...
1. **优先尝试方案 1（UI 界面配置）**
2. **如果不行，检查 API Key 有效性（方案 3）**
3. **更新 Cursor IDE 到最新版本（方案 5）**
4. **如果仍然无效，可能需要等待 Cursor IDE 的更新或修复**
